...
- Методы:
  ...
  - `impersonateUser(userId)` - войти под пользователем (сессия админа заменяется сессией пользователя; вызывающий код обязан сделать полную перезагрузку через `window.location`, возврата в админку нет — см. [Регистрация и авторизация](/processes/auth#вход-под-пользователем-из-админки))
...
- Действия: просмотр, редактирование, вход под пользователем, удаление
- Кнопка входа под пользователем показывается по флагу `can_impersonate` из API
- ConfirmDialog для подтверждения удаления и входа под пользователем
...
- Действия: редактировать, войти под пользователем (по флагу `can_impersonate`), блокировать, удалить
