...
Пункт фиксируется на заказе (`orders.pickup_point_id`) в момент оформления: тариф считается при checkout, а заявка создаётся позже (подтверждение продавца), и за это время привязка товара может измениться.
